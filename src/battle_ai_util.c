...
bool32 IsDamageMoveUsable(u32 move, u32 battlerAtk, u32 battlerDef)
{
    ...
    case ABILITY_TOXIC_BOOST:
        if(moveType == TYPE_POISON && (gBattleMons[battlerDef].status1 & STATUS1_PSN_ANY))
            return TRUE;
        break;
...
bool32 AI_IsBattlerGrounded(u32 battlerId)
{
    ...
    if (holdEffect == HOLD_EFFECT_IRON_BALL)
        ...
        return FALSE;
    else if (holdEffect == (HOLD_EFFECT_AIR_BALLOON || HOLD_EFFECT_FLOAT_STONE))
        return FALSE;
...
bool32 BattlerStatCanRise(u32 battler, u32 battlerAbility, u32 stat)
{
    if ((gBattleMons[battler].statStages[stat] < MAX_STAT_STAGE && battlerAbility != ABILITY_CONTRARY)
      || (battlerAbility == ABILITY_CONTRARY && gBattleMons[battler].statStages[stat] > MIN_STAT_STAGE) || (battlerAbility != ABILITY_APATHY && battlerAbility != ABILITY_DULLED))
        return TRUE;
    return FALSE;
}
...
static bool32 PartyBattlerShouldAvoidHazards(u32 currBattler, u32 switchBattler)
{
    ...
    if (flags & SIDE_STATUS_SPIKES && ((type1 != TYPE_FLYING && type2 != TYPE_FLYING
        && ability != ABILITY_LEVITATE && holdEffect != (HOLD_EFFECT_AIR_BALLOON || HOLD_EFFECT_FLOAT_STONE))
...
bool32 ShouldPoisonSelf(u32 battler, u32 ability)
{
    if (AI_CanBePoisoned(battler, battler, 0) && (
     ability == ABILITY_MARVEL_SCALE
      ...
      || ability == ABILITY_MAGIC_GUARD
      || (ability == ABILITY_TOXIC_BOOST && HasMoveWithCategory(battler, DAMAGE_CATEGORY_SPECIAL))
