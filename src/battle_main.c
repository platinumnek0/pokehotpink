...
void SwitchInClearSetData(u32 battler)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        if ((gStatuses4[i] & STATUS4_SLOW_SLOW_BEAM) && *(gBattleStruct->stickySyrupdBy + i) == battler)
            gStatuses4[i] &= ~STATUS4_SLOW_SLOW_BEAM;
    }
...
const u8* FaintClearSetData(u32 battler)
{
    ...
    gProtectStructs[battler].usedSoundMove = FALSE;
...
s8 GetMovePriority(u32 battler, u16 move)
{
    ...
    else if (gMovesInfo[move].effect == EFFECT_GRASSY_GLIDE && gFieldStatuses & gMovesInfo[move].argument && IsBattlerGrounded(battler))
    {
        priority++;
    }
